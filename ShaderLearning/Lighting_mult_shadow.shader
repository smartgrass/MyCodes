...
Shader "MyShader/Lighting_mult_shadow"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag(outputData i):SV_TARGET{
				...
				fixed shadow = SHADOW_ATTENUATION(i);   //获取阴影
                return fixed4( ambient +(specular + diffuse)*shadow,1.0);
            }
        ...
        Pass
        {
            ...
            #include "UnityCG.cginc"
			#include "AutoLight.cginc"
			//#pragma multi_compile_fwdadd 加阴影-> _fullshadows
			...
			fixed4 frag(outputData i):SV_TARGET{
				...
				fixed shadow = SHADOW_ATTENUATION(i);  //获取阴影, 然后混合
                return fixed4((specular + diffuse)*atten*shadow,1.0);
            }
